...
-create a program that visualize file size in the form of rectangles
...
def path_to_nested_tuple(path: str) -> tuple[str, int | list]:
    ...
    *exactly* the same, so this doctest _might_ not pass when run on
    ...
    if os.path.isdir(path):
        ...
        path_list = []
        file_dir_list = ordered_listdir(path)
        for item in file_dir_list:
            p = os.path.join(path, item)
            path_list.append(path_to_nested_tuple(p))
        return path_name, path_list
    else:
...
def dir_tree_from_nested_tuple(obj: tuple[str, int | list]) -> DirectoryTree:
    ...
    for item in obj[1]:
        if isinstance(item[1], int):
            ...
            subtree = dir_tree_from_nested_tuple(item)
            subtrees.append(subtree)
    return DirectoryTree(obj[0], subtrees, 1)
...
def moves_to_nested_dict(moves: list[list[str]]) -> dict[tuple[str,
    ...
    >>> d = moves_to_nested_dict([['a', 'b', 'c'], ['a', 'd', 'c']])
    >>> d
    {('a', 0): {('b', 0): {('c', 1): {}}, ('d', 0): {('c', 1): {}}}}
    """
    if not any(moves):
    ...
    for game in moves:
        if any(game):
            first_moves.append(game[0])
    first_moves = list(dict.fromkeys(first_moves))
    for first_move in first_moves:
        game_with_move = get_game_with_move(moves, first_move)
        value = moves_to_nested_dict(game_with_move[0])
        key = (first_move, game_with_move[1])
        output[key] = value
    return output


def get_game_with_move(moves: list[list[str]], first_move: str) -> \
        tuple[list
              [list[str]],
              int]:
    """return a list of games and its games ended that has first_move
    as its first move"""
    games_ended = 0
    game_with_move = []
    for game in moves:
        if first_move in game and first_move == game[0]:
            if len(game) == 1:
                games_ended += 1
            game_with_move.append(game[1:])
    return game_with_move, games_ended
...
class TMTree:
    ...
    def get_parent_tree(self) -> Optional[TMTree]:
        """return the parent tree of self"""
    ...
    def is_displayed_tree_leaf(self) -> bool:
        ...
        False
        """
        if not self._parent_tree or self._parent_tree._expanded:
    ...
    def get_path_string(self) -> str:
        ...
        'C | C2 | C1(5) None'
        """
        output = self._name + f"({self.data_size}) {self.rect}"
        if isinstance(self, ChessTree):
            output = output.replace(f"({self.data_size}) {self.rect}",
                                    self.get_suffix())
        curr = self
        while curr.get_parent_tree() is not None:
            curr = curr.get_parent_tree()
            output = f"{curr.get_name()} | " + output
        return output
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        if self.is_displayed_tree_leaf():
            ...
            self.update_width_greater(rect)
        else:
            self.update_height_greater(rect)

    def update_width_greater(self, rect: tuple[int, int, int, int]) -> None:
        """helper method for update_rectangle when width > height"""
        total_size = 0
        curr_width = 0
        self.rect = rect
        for subtree in self._subtrees:
        ...
        for i in range(len(self._subtrees)):
            subtree_ratio = self._subtrees[i].data_size * rect[2]
            if i == len(self._subtrees) - 1:
                self._subtrees[i].rect = (curr_width + rect[0], rect[1],
                                          rect[2] - curr_width, rect[3])
            else:
                self._subtrees[i].rect = (curr_width + rect[0], rect[1],
                                          math.floor(subtree_ratio
                                                     / total_size), rect[3])
            curr_width += math.floor(subtree_ratio / total_size)
            self._subtrees[i].update_rectangles(self._subtrees[i].rect)

    def update_height_greater(self, rect: tuple[int, int, int, int]) -> None:
        """helper method for update_rectangle when height > width"""
        total_size = 0
        curr_height = 0
        self.rect = rect
        for subtree in self._subtrees:
            total_size += subtree.data_size
        for i in range(len(self._subtrees)):
            subtree_ratio = self._subtrees[i].data_size * rect[3]
            if i == len(self._subtrees) - 1:
                self._subtrees[i].rect = (rect[0], curr_height + rect[1],
                                          rect[2], rect[3] - curr_height)
            else:
                self._subtrees[i].rect = (rect[0], curr_height + rect[1],
                                          rect[2],
                                          math.floor(subtree_ratio
                                                     / total_size))
            curr_height += math.floor(subtree_ratio / total_size)
            self._subtrees[i].update_rectangles(self._subtrees[i].rect)
    ...
    def get_rectangles(self) -> list[tuple[tuple[int, int, int, int],
        ...
        """
        output = []
        if not self._subtrees and self.is_displayed_tree_leaf():
            output.append(tuple((self.rect, self._colour)))
            return output
        if self.is_displayed_tree_leaf():
            output.append(tuple((self.rect, self._colour)))
        for subtree in self._subtrees:
            if subtree.is_displayed_tree_leaf():
                output.append(tuple((subtree.rect, subtree._colour)))
            else:
    ...
    def get_tree_at_position(self, pos: tuple[int, int]) -> Optional[TMTree]:
        ...
        >>> t3 = TMTree('C', [s1, s2], 1)
        >>> t3.update_rectangles((0, 0, 100, 200))
        >>> t3.get_tree_at_position((0, 0)) is s0
        True
        ...
        >>> a = TMTree('a', [b, c, d], 5)
        >>> a.update_rectangles((0, 0, 55, 30))
        >>> a.get_tree_at_position((5, 5)) is j
        True
        >>> a.get_tree_at_position((15, 24)) is j
        True
        """
        if pos[0] not in range(self.rect[0], self.rect[2] + 1) \
                or pos[1] not in range(self.rect[1], self.rect[3] + 1):
            return None
        else:
            displayed_leaves = self.get_rectangles()
            for leaf in displayed_leaves:
                if pos[0] in range(leaf[0][0], leaf[0][2] + leaf[0][0] + 1) \
                        and pos[1] in range(leaf[0][1],
                                            leaf[0][3] + leaf[0][1] + 1):
                    return self.get_tree(leaf)
            return None

    def get_tree(self, leaf: tuple[tuple[int, int, int, int],
                                   tuple[int, int, int]]) -> Optional[TMTree]:
        """return the tree with rect = leaf[0] and colour = leaf[1]
        for get_tree_at_position if pos is valid"""
        if self.is_displayed_tree_leaf() and self.rect == leaf[0]:
            return self
        elif self._subtrees is not None and self._expanded:
            for subtree in self._subtrees:
                output = subtree.get_tree(leaf)
    ...
    def expand(self) -> TMTree:
        ...
        if not self._subtrees:
            ...
            self._expanded = True
            curr_parent = self._parent_tree
            while curr_parent is not None:
                curr_parent._expanded = True
                curr_parent = curr_parent._parent_tree
    ...
    def move(self, destination: TMTree) -> None:
        ...
        """
        last = destination
        last._subtrees.append(self)
        self._parent_tree._subtrees.remove(self)
        old_parent = self._parent_tree
        while old_parent is not None:
            old_parent.data_size -= self.data_size
            old_parent = old_parent._parent_tree
        self._parent_tree = last
        last._expanded = True
        curr_parent = last
        while curr_parent._parent_tree is not None:
            curr_parent.data_size += self.data_size
            curr_parent = curr_parent._parent_tree
        curr_parent.data_size += self.data_size
        curr_parent.update_rectangles(curr_parent.rect)
    ...
    def change_size(self, factor: float) -> None:
        ...
        if self.data_size < max(1, subtree_size):
            self.data_size = max(1, subtree_size)
        curr_parent = self._parent_tree
        curr = self
        while curr_parent is not None:
            curr_parent.data_size += self.data_size - old_size
            curr_parent = curr_parent._parent_tree
            curr = curr._parent_tree
...
class DirectoryTree(TMTree):
    ...
    def __init__(self, name: str, subtrees: list, data_size: int) -> None:
    ...
    def move(self, destination: TMTree) -> None:
        if isinstance(destination, FileTree):
            raise OperationNotSupportedError
        elif not destination.is_displayed_tree_leaf():
            raise OperationNotSupportedError
        else:
    ...
    def get_path_string(self) -> str:
        """
        './documents/data.xlsx (file)'.replace("/", os.path.sep)
        """
        if isinstance(self, FileTree):
            s_type = ' (file)'
        else:
            s_type = ' (directory)'
        output = self._name + s_type
        curr = self
        while curr.get_parent_tree() is not None:
            curr = curr.get_parent_tree()
            output = f"{curr.get_name()}/" + output
        return output.replace('/', os.path.sep)

    def __str__(self) -> str:
        s = TMTree.__str__(self)
        output = s.replace(" | ", os.path.sep)
        return output
...
class FileTree(DirectoryTree):
    ...
    def __init__(self, name: str, subtrees: list, data_size: int) -> None:
        DirectoryTree.__init__(self, name, subtrees, data_size)
...
class ChessTree(TMTree):
    ...
    def __init__(self, move_dict: dict[tuple[str, int], dict],
        ...
        >>> moves = moves_to_nested_dict([['a', 'z'], ['b', 'z'], ['c'], ['d'], ['e'], ['f'], ['g'], ['h'], ['i'], ['j']])
        >>> ct4 = ChessTree(moves)
        >>> ct4.data_size
        10
        >>> ct4._subtrees[0].data_size
        1
        >>> ct4._subtrees[1].data_size
        1
        """
        if move_dict.values() == {}:
            keys = move_dict.keys()
            for key in keys:
                TMTree.__init__(self, key[0], [], key[1])
        else:
            ...
            for key in move_dict.keys():
                child = ChessTree(move_dict[key], key[0],
                                  not self._white_to_play, key[1])
                subtree_list.append(child)
